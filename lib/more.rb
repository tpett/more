...
class Less::More
  ...
  class << self
    attr_writer :compression, :header, :page_cache, :concat, :concat_sources, :destination_path
    ...
    # TRP: If defined and concat defined only use these source files, and in this order
    def concat_sources
      [*get_cvar(:concat_sources)]
    end
    ...
    def parse
      ...
      Less::More.all_less_files.each do |path|
        ...
        # TRP: Add up CSS
        path_as_string = path_as_array.join(File::SEPARATOR).sub(/.css$/, '')
        sum[path_as_string] = css if concat && (!concat_sources || concat_sources.include?(path_as_string))
      end
      
      # TRP: Write the sum of all generated css files to configured concat file
      if concat
        sum = concat_sources ? concat_sources.collect { |path| sum[path] }.join : sum.values.join
        write_css(concat, sum)
      end
    end
